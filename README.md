# A Thrift implementation for Elixir
...
This package contains a complete implementation of [Thrift](https://thrift.apache.org/) for Elixir. It includes a Thrift IDL parser, a code generator, a binary framed client and a binary framed server.

The serialization and deserialization code that is generated by this project is highly optimized and is between **20 and 40 times faster** than the code generated by the Apache Erlang implementation. 
...
compiler will generate Elixir source files, which will then be compiled by the `:elixir` compiler.
We currently rely on this ordering to ensure those generated source files get compiled.
...
The following examples use the following thrift definition:
...
Generated Code | Output Module
---------------|----------------
User Struct    | `Thrift.Test.User`
UserNotFound Exception | `Thrift.Test.UserNotFound`
User Binary Protocol | `Thrift.Test.User.BinaryProtocol`
UserNotFound Binary Protocol | `Thrift.Test.UserNotFound.BinaryProtocol`
UserService Framed Binary Client | `Thrift.Test.UserService.Clients.Binary.Framed`
UserService Framed Binary Server | `Thrift.Test.UserService.Servers.Binary.Framed`
UserService Handler Behviour (Used for writing servers)    | `Thrift.Test.UserService.Handler.Behaviour`
...
The static client module uses [James Fish](http://github.com/fishcakez)'s excellent [connection](http://github.com/fishcakez/connection) behaviour. 

For each function defined in the service, the generated module has four functions. Consider the `get_user_by_id` function above.
...
#### Note: in the above example, the function `deleteUser` will be converted to `delete_user` to comply with Elixr's naming conventions.
...
{:ok, client} = UserService.Clients.Binary.Framed.start_link("localhost", 2345, [])

iex(1)> {:ok, user} = Client.get_user_by_id(client, 22451)
...
-----------------|-------|-------------
`:tcp_opts` | keyword | A keyword list of tcp options
`:gen_server_opts` | keyword | A keyword list of options for the gen server 
...
alias Thrift.Test.UserService.Clients.Binary.Framed, as: Client
{:ok, client} = UserService.Clients.Binary.Framed.start_link("localhost", 2345, 
                tcp_opts: [backoff_calculator: fn(retry_count) -> retry_count * 1000], gen_server_opts: [timeout: 10_000])
                
```
In the above example, the client will use a (very) conservative backoff calculator that waits an additional second each time the client retries. In other words, the first retry will wait 1 second, the second will wait 2 seconds, and the third will wait 3. It sets the GenServer timeout to be ten seconds, which means the remote side can take its time to reply. 


## Using The Server
Creating a thrift server is slightly more involved than creating the client, because you need to create a module to handle the work. Fortunately, a `Behaviour` for this module, complete with correct success typing, is generated for you. To implement this behaviour, use the `@behaviour` module attribute. The compiler will now inform you about any missed functions.
...
defmodule UserServiceHandler do 
  ...
  def delete_user(user_id) do 
    case Backend.delete_user(user_id) do 
	   :ok -> true 
	   _   -> false
  end
end
...
After you have your handler module, you use it to build your server. 

```elixir
{:ok, server_pid} = Thrift.Test.UserService.Servers.Binary.Framed.start_link(UserServiceHandler, 2345, [])
```

...and your server is up and running. RPC calls made to the server will be delegated to your handler module. 
...
The server defines a Supervisor, which can be added to your application's supervision tree. Don't forget to use the `supervisor` function rather than the `worker` function. 
 
## Using the binary protocol directly

Each thrift struct, union and exception also has a `BinaryProtocol` module generated for it. This module lets you serialize and deserialize its own type easily. For example:
...
```
The return value of the `serialize` function is an IO List. If you need a binary, you can pipe that to `IO.iodata_to_binary`.
...
You can use these features to build in additional language suppor or to add support for additional protocols and servers.
